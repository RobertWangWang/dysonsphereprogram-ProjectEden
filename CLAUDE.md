...
powershell -ExecutionPolicy Bypass -File tools\check_fusion.ps1        # 聚变线：对撞机 100 倍功耗那条公式 + 发电机半边的四个前提（见 FusionPatches）
powershell -ExecutionPolicy Bypass -File tools\check_techbuyout.ps1    # 科技级联买断：前置判据的两张表 + 买断按钮里那两处待替换调用（见 TechBuyoutCascadePatches）
```
...
`fuelCount > 0`. **`fuelMask` does not decide whether the fuel burns** — it is the filter that
`CargoTraffic.TryPickFuel` and hand-insertion use, so a grain moved in from code bypasses it
entirely.

**That sentence used to read "`fuelMask` is not consulted at burn time", and that is measurably
too strong** — `check_fusion.ps1` reported it as a FAIL on a vanilla that was behaving exactly as
the design assumes. The field *is* read once, at `EnergyCap_Fuel` @008D, as
`if (fuelMask == 4) { if (boost) capacity *= 100; if (curFuelId == 1804) capacity *= 2; }` — the
artificial star's boost branch, reached long **after** `capacityCurrentTick` has already been
computed. So the operative claim (code-fed fuel bypasses the mask) stands, and a mod generator on
any mask other than 4 skips that branch entirely. **The lesson is the checker's, not vanilla's:
an assertion of *absence* fails the moment the field is read for an unrelated reason — assert the
shape you actually depend on.** So `RedoxBurnerPatches.Burn`, hooked into `MegaAssemblerPatches.MegaTick`, is one method
...
Units are metres: `OnNodeAdded`/`OnConsumerAdded` project positions onto a sphere of radius `realRadius + 0.2` before comparing squared distances, so planet-wide coverage needs ≥ 2 × planet radius. **`powerConnectDistance` used to be "deliberately left alone"; it is now configured per node, and the reason the old blanket rule was wrong is worth keeping.**

The cost is real and was re-measured: `PowerSystem.line_arragement_for_add_node` has **three nested loops** (backward branches spanning 349 / 303 / 17 instructions) over `List<PowerNetworkStructures.Node>.Count` — **O(n²), run once per node added**. But *what* it is quadratic in is the part the old sentence got wrong: **those loops walk the node list regardless of `connectDistance`**, which only decides how many pairs actually connect. So the direct cost does not move; what moves is that **networks merge**, and a merged network has a larger `n` — which is what feeds the n².

Two things make that acceptable for 卫星配电站 specifically. `PowerNetworkStructures.Node.conns` / `.lines` are `List<>` with **no cap**, so nothing truncates or overflows; and the cost sits on the **build path**, not the tick path.

**And leaving it at 0 had a correctness cost that outweighed it: coverage is not connectivity.** Two nodes share a network through `conns`, not through overlapping coverage circles — so with `connectDistance = 0`, two satellite substations on one planet formed **two disjoint grids** that did not share power, while both visibly covered the whole planet. That reads to a player as "it's covered but the power doesn't flow". Both of its ranges are now 2000 m. The blanket warning still stands for 电力感应塔, which is built in the hundreds; its connect distance is deliberately only 2× its coverage, not planet-wide.
