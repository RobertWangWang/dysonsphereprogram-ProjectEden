...
| **物品品质** | 同一种金属现在可以**更纯**，而纯度是**一格货的属性**，不是另一个物品——DSP 没有逐件元数据可挂，所以它做成了可加的点数，和增产点数同构：合并相加、拆分按件数带走，加权平均自动成立。**唯一来源是同位提纯厂**，采出来的矿品质恒为 0。三条配方各是一道真实工艺（电解精炼 / 区域熔炼 / 羰基提纯），收率 80% → 40% → 15%，**收率就是成本**。三级**全部万用**：提哪一种金属由每台建筑自己在面板上选，候选清单是从矿脉和原版配方**推导**出来的，加新矿会自己长。⚠️ **目前品质能产、能流、能存、能看，但还不产生任何效果**，效果层是下一步 |
