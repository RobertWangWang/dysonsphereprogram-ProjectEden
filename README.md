...
| **可燃液体发电** | 13 种液体各有**工作温度**，决定一份燃料里有多少真的变成电——**脏的能量密、干净的能量稀**——藻油 2.6 MJ / 250 ℃ 到氨 2.6 MJ / 2000 ℃ 之间隔着十二档。（同一桶原油的四个切段则是一条**纯温度梯子**：热值全部相同，只有允许的工作温度从 500 ℃ 排到 900 ℃——每件含碳量一样，燃烧焓就一样）。**原油本身也能直接烧**，但它是最差的一档（400 ℃）：未脱盐的整桶，钒、硫、盐、水俱全。直接烧 10 桶得 15.8 MJ 电，先蒸馏再各自烧得 21.5 MJ——**精炼一遍多 36%**，所以它是「能烧但划不来」的粗放燃料，没有废掉炼油线 |
| **改动的原版配方** | **处理器**除原有原料外还要吃 **电磁矩阵 ×2**。这是一条**平衡改动，不是从化学推出来的**：处理器下游极广，给它挂矩阵依赖等于把研究站提前拉进产线——有意为之。老存档直接能用（投料槽 2 → 3 由原版自己的 `Array.Resize` 兜住，旧料保留、新槽从 0 开始）。不想要就把 `recipes.json` 的 `vanillaEdits` 清空，不用重新编译 |
