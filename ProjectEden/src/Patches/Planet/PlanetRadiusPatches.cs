...
namespace ProjectEden.Patches
{
    ...
    internal static class PlanetRadiusPatches
    {
        ...
        internal static void Report()
        {
            ...
            {
                ProjectEdenPlugin.Log.LogInfo(
                    "行星放大：**已关闭**（默认值）。开了之后行星半径 200 → 400、"
                    + "可建造面积约 4 倍。**只在全新存档上开，而且开了就不要再改这个数**"
                    + "——建筑坐标的模长约等于半径，改一次等于重开档。"
                    + $"开关在 {where} 的 enabled");
            ...
            ProjectEdenPlugin.Log.LogWarning(
                ...
                + $"可建造格位约 {actual * actual:0.0} 倍。"
                + "**这是不可逆的：建筑坐标的模长约等于半径，所以改倍率或者关掉开关，"
                + "都等于让已有存档里每一座建筑都站在错误的高度；而老存档的地形改造数据"
                + "长度对不上，第一次读地形就会直接崩。只在全新存档上开。**"
