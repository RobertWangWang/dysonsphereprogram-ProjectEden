...
namespace ProjectEden.Utils
{
    ...
    internal class MachineFullVariantEntry
    {
        ...
        /// <summary>
        /// 燃料类型。0 则继承源物品（原版是 8）；<b>−1 表示它根本不是燃料</b>。
        ///
        /// 需要这第三档，是因为容量倍率一大，「顺带也是机甲燃料」就变成白送的数值膨胀：
        /// 原版满蓄电器 540 MJ 能进机甲反应堆，容量放大几百倍之后那就是无限续航。
        /// 而容量本身是要放大的——高密度储能这条线的全部意义就在那里。
        /// <b>容量和「能不能当机甲燃料」必须能分开配</b>，否则只能在
        /// 「电池不够大」和「机甲开挂」之间二选一。
        /// </summary>
        public int fuelType;
    ...
    internal class MachineStationEntry
    {
        ...
        /// <summary>
        /// 这台物流站是个<b>垃圾箱</b>：每 tick 把储物格清空，货直接消失。
        ///
        /// <b>为什么是物流站而不是储物仓。</b> 这个 mod 的废料堆在物流网里，不在某一台
        /// 机器旁边。物流站本身就带三条进货路径——传送带口、行星内运输机、星际运输船——
        /// 把一格设成「本地需求」就等于「把全行星的这种货收过来」，设成「远程需求」
        /// 就是跨星系收。所以垃圾箱只需要借这三条路，自己一条喂料逻辑都不用写。
        ///
        /// 销毁是<b>纯销毁</b>：不产出任何东西。给产物就会多出一条「废料 → 某种资源」的
        /// 通路，而废料在 10000× 的工厂里是无限的，那等于开了个无中生有的口子。
        /// 销毁量会记进本行星的 <c>consumeRegister</c>，所以生产面板上看得到扔掉了什么。
        ///
        /// <b>翘曲器那一格不清。</b> <c>StationComponent.InternalTickRemote</c> @0044–007B
        /// 是「扫 storage 找 itemId == 1210，每 tick 搬一个进 warperCount」——
        /// 也就是说翘曲器是<b>走储物格进来的</b>。跟着一起清掉的话，垃圾箱自己的星际船
        /// 永远攒不够翘曲器，而症状是「它收货特别慢」，指不到病因上。
        /// </summary>
        public bool voidItems;

        /// <summary>
        /// 每 10 秒往日志里打一行「这颗星球的垃圾箱这一轮销毁了多少」。平时关着，别刷屏。
        ///
        /// 关着也仍然有两行：开机的状态行（接没接上），以及每颗星球<b>第一次</b>真的销毁
        /// 东西时的那一行。少了后者就分不开「补丁没生效」和「玩家还没往里放货」——
        /// 本文件记过七次的那条，状态行和事件行谁都替不了谁。
        /// </summary>
        public bool voidDebugLog;
    }
}
