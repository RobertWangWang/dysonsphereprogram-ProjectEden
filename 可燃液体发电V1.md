...
flowchart TD
    ...
    subgraph FUEL["六种可燃液体（温度 / 热值 两个属性）"]
        F1["藻油<br/>250°C　4.2 MJ"]
        F2["钒渣油<br/>500°C　12.0 MJ"]
        F3["精炼油<br/>750°C　4.4 MJ"]
        F4["乙烯<br/>1000°C　9.7 MJ"]
        F5["甲醇<br/>1500°C　5.0 MJ"]
        F6["氨<br/>2000°C　2.6 MJ"]
    end
...
(1804 = **金色燃料棒**,由 `FuelSurvey` 在游戏里读出来的;和它同属 `FuelType 4` 的还有
反物质燃料棒。也就是说人造恒星烧金色燃料棒时功率直接翻倍。)
...
**这张表和初稿不一样,两处都是实现时改的,理由在 §9.2 和这里。**

初稿排了「藻渣浆液」和「无水肼」两种新物品。落地时换成了已经存在的**藻油**和**乙烯**:
藻渣浆液和藻油在这条链上是同一个东西(生物油,碱金属重),而无水肼在游戏里没有上游——
它要过氧化氢或次氯酸钠,两者都不存在,硬造就得再开两条化工链。
**换完之后只需要新增一种物品。**

| 温度 | 液体 | 温度的依据 | 热值 | 热值的依据 |
|---|---|---|---|---|
| 250 °C | **藻油**(已有) | 碱金属 + 高水分,结渣最严重 [11][12][14] | 4.2 MJ | 已在表里 |
| 500 °C | **钒渣油**(新增) | V₂O₅ 热腐蚀 [1][4][5][6][20] | 12.0 MJ | **无固定化学式**,见下 |
| 750 °C | **精炼油**(原版) | 含硫馏分;对应 1375 °F 实测限值 [2][3] | 4.5 MJ | 原版值,不改;**这个数是启动时读出来的** |
| 1000 °C | **乙烯**(已有) | **积碳前驱体**,C₂H₄ 是生成炭黑的标准燃料 | 9.7 MJ | 1411 kJ/mol,已在表里 |
| 1500 °C | **甲醇**(已有) | 分子里没有 C–C 键,**形不成炭黑** | 5.0 MJ | 726 kJ/mol,已在表里 |
| 2000 °C | **氨**(已有) | 无碳无灰无硫,上限最高 [7][8] | 2.6 MJ | 382.6 kJ/mol,已在表里 |

洁净度阶梯是通顺的:**碱金属灰 → 金属氧化物热腐蚀 → 硫与芳烃 → 积碳 → 无碳黑 → 无碳**。

**钒渣油的热值没有配平方程。** 减压渣油是个馏分不是化合物,和 `残渣提钒` 是同一种处理:
现实里没有方程可抄,就说清楚没有,而不是编一个。12.0 MJ 是平衡旋钮,定在乙烯
(9.7 MJ,C2 分子)之上,因为它是 C20 以上的重组分。

热值对温度**真的非单调**——最高在 500 °C,最低在 2000 °C,而且除钒渣油外全是既有的
燃烧焓换算值,一个都没为了这条链调过。
...
- **`modelId`**:708–727 **已经占满了**,而 `ResolveModelId` 是从
  ...
  按惯例扫出来之后钉进 JSON。
- **`FuelType`** 的新位:**用 16**。`FuelSurvey` 实测原版已用位掩码是 15
  (1 化学燃料 26 种 / 2 氘核燃料棒 / 4 反物质与金色燃料棒 / 8 蓄电器满),
  bit 0~5 里只剩 **16 和 32** 两个空位。取 16,把 32 留着。
  注意 `FuelType 8` 里已经有本 mod 的 锂电池蓄电器（满）(6534) —— 说明加新燃料进
  现有位是可行的,但那样原版对应的机器也会烧它,所以这里必须开新位。

---

## 八、三件要先验的事——都验完了

按仓库的规矩,写在这里而不是假装已经知道。**三条现在都有答案**:8.1 离线读 IL 解决,
8.2 的探针跑了一次游戏,结果在 8.3——其中一条**推翻了本文先前的说法**。
...
| 建筑 | 发电 | 耗料 | efficiency | `fuelMask` |
|---|---|---|---|---|
| 火力发电厂 (2204) | 2.16 MW | 2.7 MW | **0.8** | 1 |
| 微型聚变发电站 (2211) | 15 MW | 15 MW | 1.0 | 2 |
| 人造恒星 (2210) | 72 MW | 72 MW | 1.0 | 4 |

**原版火力发电厂不是 1:1,是 0.8。** 本文先前写的「煤 2.7 MJ ÷ 2.16 MW = 1.25 s
看着就是 1:1」把方向推反了:实际是每秒烧掉 2.7 MJ 的燃料能量、发出 2.16 MJ 的电,
一份煤只烧 **1.0 秒**,玩家拿到的是 2.16 MJ 而不是 2.7 MJ。

**这一条比修正数字更有用:原版唯一有转化损失的,就是烧化学燃料那一座。**
聚变和反物质都是 1.0。所以「化学燃烧有损失、而且损失随燃料变」不是本 mod 引入的
新概念,它是原版已有模式的延伸——只不过原版把损失固定成一个数,这里让它跟着温度走。
...
### 9.1 实测的表(配置落地后从 JSON 算出来的)

电厂 216 MW(火力发电厂 ×100),prefab 效率锚在本表最高的 0.6082——脚本核过,对得上。

| 温度 | 燃料 | 热值 | η | **单件出电** | `useFuel` 倍率 | 件/秒 |
|---|---|---|---|---|---|---|
| 250 °C | 藻油 | 4.2 MJ | 0.3011 | 1.26 MJ | ×2.020 | 170.8 |
| 500 °C | 钒渣油 | 12.0 MJ | 0.4301 | 5.16 MJ | ×1.414 | 41.9 |
| 750 °C | 精炼油 | 4.5 MJ | 0.4960 | 2.23 MJ | ×1.226 | 96.8 |
| 1000 °C | 乙烯 | 9.7 MJ | 0.5361 | 5.20 MJ | ×1.135 | 41.5 |
| 1500 °C | 甲醇 | 5.0 MJ | 0.5823 | 2.91 MJ | ×1.044 | 74.2 |
| 2000 °C | 氨 | 2.6 MJ | 0.6082 | 1.58 MJ | ×1.000 | 136.6 |

热值对温度是**真的非单调**——最高的那个在 500 °C,最低的在 2000 °C。这一点没有调过,
除钒渣油外全是既有的燃烧焓换算值。

> 精炼油那一行原先写的是 4.4 MJ,那是从 `ores.json` 一条注释里抄来的。
> `FuelSurvey` 启动时读出来的实际值是 **4.5 MJ**,已经改过来,连同那条注释一起。
> **注释里的依据也会过期**,这是本条链上第二次被日志纠正(第一次是原版火力发电厂的 0.8)。

### 9.2 一处被改正的化学:甲醇比乙烯干净

初稿把乙烯排在 1500 °C、甲醇排在 1000 °C,理由是「乙烯是更洁净的轻烃」。**这是错的。**

**乙烯是典型的积碳前驱体**——燃烧研究里 C₂H₄/C₂H₂ 正是生成 PAH 与炭黑的标准燃料
(HACA 机理)。而**甲醇分子里没有 C–C 键,根本形不成炭黑**,火焰干净到几乎看不见。
所以按「积灰限制温度」这条判据,甲醇应当排在乙烯之上。

改过来之后整张表的洁净度阶梯才是通顺的:
**碱金属灰 → 金属氧化物热腐蚀 → 硫与芳烃 → 积碳 → 无碳黑 → 无碳**。

### 9.3 支配扫描:和当年的金属表一模一样

拿「单件出电」和「η」两维做支配扫描,**八对被支配**,帕累托前沿只剩
{钒渣油、乙烯、甲醇、氨};藻油被全部五种支配,精炼油被两种支配。

这**不是**说这两种是死内容——它是 `cost` 那个老问题。CLAUDE.md 里记着同一件事:

> 裸的四维支配扫描扫 21 种材料报 13 对,毫无用处;加上 cost 列只剩 1 对。

这里的 cost 是真实存在的,而且有些是结构性的:

- **乙烯是用甲醇做的**(MTO)。所以甲醇在任何意义上都不可能被乙烯支配——
  你得先有甲醇才换得到乙烯,换这一步本身就是净亏。
- **藻油出自生物温室**,而那座建筑整座受日照约束,天然只有约一半工时。
- **精炼油是全游戏最便宜、最容易上量的液体**,它的位置本来就是「保底」而不是「最优」。
- **钒渣油那条配方是故意比等离子精炼亏的**,只有专门要渣油时才跑。

**所以标定的形式不是改这两个数,是把成本这一维显式算出来再扫一遍。**
最直接的做法是沿配方图往上折算到原料,得到「一件液体折合多少原矿/原油/水」,
当成第三维加进扫描——和金属表加 cost 列是同一个动作。**这件事还没做。**

### 9.4 另一条刹车:件/秒

DSP 的瓶颈通常是**件/秒**而不是焦耳。上表最右一列的跨度是 41.5(乙烯)到
170.8(藻油),**4.1 倍**——也就是说烧藻油要建 4 倍于烧乙烯的上游产能。
这条刹车不依赖玩家算账,而且 `genEnergyPerTick` 定得越大它越硬。

注意本 mod 的传送带集装是 5000 层,**带宽根本不是约束**,所以这里的「件/秒」
指的是产线吞吐,不是传送带。

### 9.5 效率低于原版这件事

顶格 η = 0.608 **低于原版火力发电厂的 0.8**
(§8.3 实测)。差距是 1.32 倍,不是原先估的 1.64 倍——但方向没变。
这是有意选的,它意味着这座建筑的卖点**不能是效率**,只能是:
