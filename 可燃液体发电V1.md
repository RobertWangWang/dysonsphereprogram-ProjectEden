...
按仓库的规矩,写在这里而不是假装已经知道。

1. **原版火力发电厂是不是 1:1。** 第九节里「顶格 0.61 仍低于原版」这句依赖它。
   煤 2.7 MJ ÷ 2.16 MW = 1.25 s 看着就是 1:1,但 prefab 的两个值读不到,
   要在游戏里打一行日志确认。
2. **哪个 `FuelType` 位是空的**(见 §7.4)。
3. **异常检测会不会在别处也拿 prefab 比。** `ABN_PowerGenerator` 还有个
   `BeforeGameSave` 也读这两个字段,IL 还没读过。
